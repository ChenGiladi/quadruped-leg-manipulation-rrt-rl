...
# --------------------------------------------------------------------------
# Config: override with environment variables on any OS.
#   COPPELIASIM_PY_PATH  -> CoppeliaSim's programming/zmqRemoteApi/clients/python
#   RL_PUSH_LOG_DIR      -> directory for training logs / CSV traces
# --------------------------------------------------------------------------
_DEFAULT_COPPELIA_PATHS = [
    ...
    os.path.expanduser("~/CoppeliaSim/programming/zmqRemoteApi/clients/python"),
]
_coppeliasim_python_dir = os.environ.get("COPPELIASIM_PY_PATH")
if not (_coppeliasim_python_dir and os.path.isdir(_coppeliasim_python_dir)):
    _coppeliasim_python_dir = next(
        (p for p in _DEFAULT_COPPELIA_PATHS if os.path.isdir(p)), None
    )
if _coppeliasim_python_dir is None:
    raise RuntimeError(
        "CoppeliaSim Python client directory not found. Set the COPPELIASIM_PY_PATH "
        "environment variable to <CoppeliaSimRoot>/programming/zmqRemoteApi/clients/python."
    )
sys.path.append(os.path.abspath(_coppeliasim_python_dir))
...
csv_file = os.path.join(log_dir, "train_data.csv")
...
class RobotModelEnv(gym.Env):
    ...
    def set_box_mass_kg(self, kg):
        """Set the mass (kg) of the box (CUBE) in the running CoppeliaSim scene; used by the Figure-17 mass-robustness sweep."""
        sim.setShapeMass(CUBE, float(kg))
    ...
    def __init__(self, action_type='discrete'):
        ...
        cube_high_position = np.array([12.5, 12.5, 1], dtype=np.float32)
        cube_low_position = np.array([-12.5, -12.5, -0.1], dtype=np.float32)
        ...
        # high_action = np.array([0.5, 0.5, 1.0], dtype=np.float32)
        # low_action = np.array([0.01, 0.01, 0.0], dtype=np.float32)
        high_action = np.array([8, 8, 2], dtype=np.int)
        low_action = np.array([1, 1, 1], dtype=np.int)
        # self.action_space = spaces.Box(low=low_action, high=high_action, shape=(3,), dtype=np.float32)
        # self.action_space = spaces.Box(low=low_action, high=high_action, shape=(3,), dtype=np.int)
    ...
    def step(self, action):
        ...
        # X_Bez_1 = float(action[0])
        # X_Bez_2 = float(action[1])
        # leg_num = action[2]
        # if leg_num >0.5:
        #     leg_num = 2
        # else:
        ...
        if fall or dist_1_NROM<0.5 or dist_2_NROM<0.5 or np.abs(dist_1[0])<0.1 or np.abs(dist_2[0])<0.1:
            done = True
            ...
            #     writer = csv.writer(f)
            #     writer.writerow([self.csv_row, self.cum_reward[-1], fall])#, self.passed_stair_flag, self.reach_target_flag])
            ...
            if episode % 10 == 0 or episode == 1:
                ...
                file_name = os.path.join(log_dir, 'train_data_' + str(episode) + '_.csv')
                ...
                file_name2 = os.path.join(log_dir, 'X_Y_TRAJ_' + str(episode) + '_.csv')
